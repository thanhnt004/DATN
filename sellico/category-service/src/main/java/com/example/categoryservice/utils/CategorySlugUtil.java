package com.example.categoryservice.utils;
...
public final class CategorySlugUtil {

    private CategorySlugUtil() {
     ...
     * @param parentId     parent category (null = root)
     * @param exists       (parentId, slug) -> true nếu đã tồn tại
     */
    public static String uniqueCategorySlug(
            String categoryName,
            UUID parentId,
            BiPredicate<UUID, String> exists
    ) {
        return uniqueCategorySlug(categoryName, parentId, exists, 0, "-");
    }
    ...
    public static String uniqueCategorySlug(
        ...
        String base = slugify(categoryName, separator, maxLen, true);

        if (!exists.test(parentId, base)) return base;

        for (int i = 2; i <= MAX_UNIQUE_ATTEMPTS; i++) {
            String candidate = appendSuffix(base, i, maxLen, separator);
            if (!exists.test(parentId, candidate)) return candidate;
        }
